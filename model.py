...
class Review_Data_Dataset(Dataset):
    def __init__(self, tokenized_data, tokenizer):
        ...
        self.encodings_word = [
            tokenizer(sentence[0], truncation=True, padding='max_length', max_length=128, return_tensors="pt")  for sentence in tqdm(tokenized_data, desc="Encoding Words")
        ]
        self.encodings_pos = [
            tokenizer(" ".join([token[0] for token in sentence[1]]), truncation=True, padding='max_length', max_length=128,return_tensors="pt")
...
class Feature_Aug_Classify(nn.Module):
    def __init__(self,model, model_path,hidden_size,head_num,pos,content,dependency):
        ...
        self.gcns_word = nn.ModuleList([GCN(768, hidden_size, True) for _ in range(head_num)])
        ...
        self.classify = Reiveiw_Classify(hidden_size * count)
        self.bn = torch.nn.BatchNorm1d(hidden_size*count)
        self.drop = nn.Dropout(0.1)

    def forward(self, content_input, pos_input, dependency_input):
        logit_content = None
        logit_pos = None
        logit_dependency = None
        ...
        if self.content:
            ...
            feature_content = self.process_attention(content_attention, content_hidden, self.gcns_word, content_input['attention_mask'])
            outputs_to_concat.append(feature_content)
        # Pos
        if self.pos:
            ...
            outputs_to_concat.append(feature_pos)
        # Dependency
        if self.dependency:
            ...
            feature_dependency = self.process_attention(dependency_attention, dependency_hidden, self.gcns_dependency, dependency_input['attention_mask'])
            outputs_to_concat.append(feature_dependency)
        if outputs_to_concat:
            output = torch.cat(outputs_to_concat, dim=1)
    ...
    def process_attention(self, attentions, hidden_state, gcns, atten_mask):
        ...
        for head in range(self.head_num):
            ...
            word_out_heads.append(out)
        output = torch.stack(word_out_heads, dim=1)
